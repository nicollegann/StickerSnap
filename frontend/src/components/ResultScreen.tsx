...
const SHARE_ACTIONS: ShareAction[] = [
  { id: 'save',     label: 'Save',     icon: '↓', color: '#2D9E6B' },
  { id: 'copy',     label: 'Copy',     icon: '⊕', color: '#5B7FFF' },
...
export function ResultScreen({ stickerUrl, onReset }: ResultScreenProps) {
  ...
  const [actionStates, setActionStates] = useState<Record<ShareTarget, ActionState>>({
    save: 'idle', copy: 'idle', telegram: 'idle', whatsapp: 'idle', share: 'idle',
  });
          ...
          return (
            ...
            >
              <span className="result-screen__action-icon">
                {state === 'loading' ? '…' : state === 'done' ? '✓' : action.icon}
              </span>
              <span className="result-screen__action-label">{action.label}</span>
            </button>
          );
        })}
      </div>
